...
function RegularMeetingContent({ id }: { id?: string }) {
  ...
  const context = useOutletContext<{ challenge?: ChallengeInfo } | undefined>();

  if (isLoading) return <RegularMeetingSkeleton />;
  if (!data) return <div className={styles.emptyState}>모임을 찾을 수 없습니다.</div>;

  const isLeader = context?.challenge?.myMembership?.role === 'LEADER';
  const canRespond = data.status !== 'COMPLETED';
  const canComplete = isLeader && data.status !== 'COMPLETED';
  const shouldShowActions = canRespond || canComplete;
  const locationLabel = data.displayLocation || (data.isOnline ? '온라인 모임' : data.location);

  return (
    <>
      <div className={cn(styles.contentStack, shouldShowActions && styles.withFloating)}>
      ...
      {shouldShowActions ? (
        <div className={styles.floatingContainer}>
          <div className={cn(styles.floatingInner, canComplete ? styles.twoActions : styles.singleAction)}>
            {canRespond ? (
              <Button
